...
def test_silence_after_answer_is_suspicious():
    v = synthesise(
...
def test_ringback_after_answer_locks_fas():
    ...
    # Score: ringback(3) -> SUSPICIOUS (>=2 threshold at balanced)
    assert v.verdict in ("SUSPICIOUS", "LIKELY_FAS")
    assert any("ringback" in r.lower() for r in v.reasons)
    # SUSPICIOUS confidence calibrates 0.3..0.65; ringback alone sits at the
    # low end of that range. Just check it's not below the INCONCLUSIVE floor.
...
def test_fingerprint_plus_silence_plus_aasist_is_fas():
    ...
    # +3 fp +2 silence +2 aasist = +7 -> LIKELY_FAS
...
def test_aggressive_preset_fires_easier_than_balanced():
    ...
    severity = {"LIKELY_REAL": 0, "INCONCLUSIVE": 1, "SUSPICIOUS": 2, "LIKELY_FAS": 3}
