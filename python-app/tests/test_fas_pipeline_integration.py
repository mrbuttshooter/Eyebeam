...
def test_pure_silence_clip_is_suspicious():
    clip = _silence(4.0)
    ...
    # silence-only (+2) hits the SUSPICIOUS threshold (2). Some test runs
    # see silence_score short-circuit which sets stability=1.0; the rules
    # engine treats high-stability without other context as neutral.
...
def test_realistic_speech_with_high_vad_is_real():
    ...
    # Expect at minimum NOT to be LIKELY_FAS / SUSPICIOUS; INCONCLUSIVE
    # or LIKELY_REAL both pass.
    assert v.verdict in ("INCONCLUSIVE", "LIKELY_REAL")
